...
const ShopWithSidebar = () => {
  ...
  const categoryHandler = (cateSelect) => {
    const cateFilterProduct = cateProduct.filter(
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "https://qwikmart.pythonanywhere.com/products/",
          {
            method: "GET",
            headers: {
        ...
        setcateProduct(dataArray);
        setFilterProduct(dataArray);
      } catch (error) {
        ...
        <div className="col-lg-9">
          <div className="row row--15">
            {filterProduct.length > 0 ? (
              filterProduct.slice(0, productShow).map((data) => (
